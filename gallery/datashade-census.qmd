---
title: "The US Census, all 306 million points"
description: "Datashading the full 2010 US Census — population density, and an additive per-race composite — reproducing the classic datashader example."
categories: [Big data]
image: figs/census-thumb.png
---

This is the showcase for `mark_datashade()`: one point per person from the 2010
US Census — about **306 million rows** — rendered as a map. It reproduces
[datashader's census example](https://examples.holoviz.org/gallery/census/census.html)
with vellumplot.

::: {.callout-note}
This example is **not run** when the site is built. It needs the `arrow` and
`dplyr` packages and a one-time ~1.44 GB download, and it wants several GB of RAM
for the full dataset. The images below are the real output, rendered once at
6000 × 4200 px and committed with the site. The script below is the exact,
runnable source ([`inst/examples/19-datashader-census.R`](https://github.com/r-vellum/vellumplot/blob/main/inst/examples/19-datashader-census.R)
in vellumplot).
:::

## Population density

Every person, coloured by local density on a single ramp — the classic "where
do people live" map. `how = "eq_hist"` (histogram equalisation) spreads the
enormous dynamic range across the colour ramp so both empty plains and dense
cities stay legible.

![US population density (2010 Census, ~306M points)](figs/census-density.png){fig-alt="Datashaded map of US population density from the 2010 census, ~306 million points, on a black-to-white heat ramp"}

## Coloured by race

There is no single-call categorical shading, so this reproduces datashader's
`count_cat` by hand: one `mark_datashade()` layer per race, each ramping from
black to its own hue, all composited additively with `blend = "screen"`. Where
populations overlap, the hues mix.

![US population by race (2010 Census)](figs/census-race.png){fig-alt="Datashaded map of US population by race from the 2010 census, one additively-blended hue per group"}

## The script

The full source, verbatim. It aggregates at `gw = 5000` cells across the
contiguous-US window and renders a 6000 px-wide page so the datashade raster is
drawn close to 1:1.
