...
hand. Here is one `render_spec` call, sent to the server as a process and read back:

```{r}
csv <- file.path(tempdir(), "penguins.csv")
write.csv(peng, csv, row.names = FALSE)

ask <- function(name, arguments) {
  req <- list(jsonrpc = "2.0", id = 1L, method = "tools/call",
              params = list(name = name, arguments = arguments))
  out <- system2("Rscript", c("-e", shQuote("vellumplot::mcp_serve()")),
                 input = jsonlite::toJSON(req, auto_unbox = TRUE), stdout = TRUE)
  jsonlite::fromJSON(out, simplifyVector = FALSE)$result
}

res <- ask("render_spec", list(spec = spec, data_path = csv,
                               out_path = "figs/agent-specs-mcp.png"))
cat(res$content[[1]]$text)
```
...
and the host never ran any. A spec the model got wrong comes back the same shape as the
error above, flagged as a tool error so the client knows to repair rather than report:

```{r}
res <- ask("render_spec", list(spec = sub("body_mass", "mass", spec, fixed = TRUE),
                               data_path = csv))
res$isError
cat(res$content[[1]]$text)
```
...
Each exchange above is its own `Rscript` process. A long-lived client keeps the pipe
open and reuses the session.
:::
